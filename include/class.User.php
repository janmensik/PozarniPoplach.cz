...
class User extends Modul
{
    ...
    private function processUser(): void
    {
        if (isset($this->user['page_schema']) && is_string($this->user['page_schema'])) {
            $schemaStr = stripslashes($this->user['page_schema']);
            if (strpos($schemaStr, 'a:') === 0 || strpos($schemaStr, 'O:') === 0) {
                $this->user['page_schema'] = @unserialize($schemaStr, ['allowed_classes' => false]);
            } else {
                $this->user['page_schema'] = json_decode($schemaStr, true);
            }
        }
