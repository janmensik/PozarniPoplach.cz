...
class User extends Modul
{
    ...
    private function processUser(): void
    {
        if (isset($this->user['page_schema']) && is_string($this->user['page_schema'])) {
            ...
            $stripped_schema = stripslashes($raw_schema);
            if (strpos($stripped_schema, 'a:') === 0 || strpos($stripped_schema, 'O:') === 0) {
                $this->user['page_schema'] = @unserialize($stripped_schema, ['allowed_classes' => false]);
            } else {
                $this->user['page_schema'] = @json_decode($raw_schema, true);
            }
        }
