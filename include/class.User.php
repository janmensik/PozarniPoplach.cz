...
class User extends Modul
{
    ...
    private function processUser(): void
    {
        if (isset($this->user['page_schema']) && is_string($this->user['page_schema'])) {
            $raw_schema = stripslashes($this->user['page_schema']);
            if (strpos($raw_schema, 'a:') === 0 || strpos($raw_schema, 'O:') === 0) {
                $this->user['page_schema'] = @unserialize($raw_schema, ['allowed_classes' => false]);
            } else {
