...
class User extends Modul
{
    ...
    private function processUser(): void
    {
        if (isset($this->user['page_schema']) && is_string($this->user['page_schema'])) {
            $schema_string = stripslashes($this->user['page_schema']);
            if (strpos($schema_string, 'a:') === 0) {
                $this->user['page_schema'] = @unserialize($schema_string, ['allowed_classes' => false]);
            } else {
                $this->user['page_schema'] = json_decode($schema_string, true);
            }
        }
