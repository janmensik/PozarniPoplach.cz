...
class User extends Modul
{
    ...
    private function processUser(): void
    {
        if (isset($this->user['page_schema']) && is_string($this->user['page_schema'])) {
            $raw = stripslashes($this->user['page_schema']);
            if (strpos($raw, 'a:') === 0 || strpos($raw, 'O:') === 0) {
                $this->user['page_schema'] = @unserialize($raw, ['allowed_classes' => false]);
            } else {
                $this->user['page_schema'] = @json_decode($raw, true);
            }
        }
